...
abort "unexpected essentials version" unless essentials.fetch("version") == "0.3.0"
abort "description must show version" unless essentials.fetch("desc").include?("[v0.3.0]")
...
expected_tiles = ["🩺 Essentials.Health.v0.3.0"]
...
abort "health tile URL must include version" unless providers.fetch("🩺 Essentials.Health.v0.3.0").fetch("url").end_with?("?v=0.3.0")
