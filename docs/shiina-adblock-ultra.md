...
- 百度贴吧改为保守模式：不再改写首页、吧内列表、帖子详情、评论、看图和同步接口。
- 移除对 `sync` / `newRnSync` / `mlog` 的直接拒绝，避免影响 App 初始化和页面加载。
- 仅保留开屏广告、明确广告接口与广告素材规则。

## v1.2.1 调整

- 菜鸟裹裹的 `show` / `batch.show` 开屏广告接口改为返回标准 MTop 空成功响应，避免直接拒绝请求后 App 回退到旧广告缓存。
- `mshow` 仅清理带明确广告展示字段的结构，不修改包裹、物流、取件和寄件数组。
- 补充菜鸟新观察到的 MTop 主机，不引入 HTTPDNS 改写或宽泛阻断。

## v1.2.0 调整

- 从 `ddgksf2013/StartUpAds` 的差异中选择性加入 17 条接口语义明确的开屏或广告规则。
- 继续排除银行证券、宽泛 RPC、任意主机匹配、地区修改和非广告响应体替换。
- B 站加入旧 Quantumult X 配置中的 1080P/高码率账户能力，仅接管 `account/mine` 与 `account/myinfo`，不接管播放地址。
