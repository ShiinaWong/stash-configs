...
- “我的”页面推广入口精简
- 顶部活动入口清理
- 在 `account/mine` 与 `account/myinfo` 响应中启用旧 Quantumult X 规则的 1080P/高码率能力
...
1080P 能力通过修改账户响应中的 VIP 能力字段实现，但不接管播放地址。这样仍可避免脚本进入评论、详情、相关推荐和播放链路。如果遇到评论、视频加载或播放异常，请先停用本覆写再复测。
...
- `v0.4.0`：按原 Quantumult X 配置加入 `account/mine` 与 `account/myinfo` 的 1080P/高码率能力；不扩大到评论、详情、相关推荐、播放或 gRPC 链路。
